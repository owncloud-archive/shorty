...
try
{
	$mode  = Type::req_argument ( 'mode', Type::STRING, TRUE );
	...
	switch ( $mode )
	{
		...
		case 'favicon':
			$hash = Type::req_argument ( 'hash', Type::STRING, FALSE );
			if ($hash) {
				...
				$url = Type::req_argument ( 'subject', Type::URL, TRUE );
				// re-hash the url and compare the result with the specified hash
				if ( ! Tools::checkSubjectHash ($url, $hash) ) {
