...
class Exception extends \Exception
{
	...
	{
		return L10n::t ( $this->phrase, $this->param );
	}
	...
	static function JSONerror ( Exception $e )
	{
		...
		switch ( get_class($e) )
		{
			...
			case 'PDOException':
				$message = sprintf ( L10n::t( "%s\nMessage(code): %s (%s)\nFile(line): %s (%s)\nInfo: %%s",
											L10n::t("Exception (%s)", get_class($e)),
			...
			default:
				if ( is_a($e,'Exception') )
					$message = L10n::t("Unexpected type of exception caught:%s\n%s", get_class($e), $e->getMessage());
				else $message = L10n::t("Unexpected thrown object of type caught:\n%s", get_class($e));
		} // switch
		// swallow any accidental output generated by php notices and stuff to preserve a clean JSON reply structure
		$output = trim ( Tools::ob_control(FALSE) );
		if ( $output )
		{
			$message = "! Swallowing accidental output from ajax routines ! \n"
...
class HttpException extends Exception
{
	...
	public function __construct ( $status )
	{
		if (   is_numeric($status)
			&& array_key_exists($status,Type::$HTTPCODE) )
		{
			$status = intval($status);
			$phrase = Type::$HTTPCODE[$status];
		}
		else
		{
			$status = 400;
			$phrase = Type::$HTTPCODE[400]; // "Bad Request"
		} // else
		...
		$tmpl->assign("explanation", L10n::t($phrase));
		$tmpl->printPage();
		exit;
  } // function __construct
