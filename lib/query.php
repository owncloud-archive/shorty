...
class OC_ShortyTracking_Query
{
	...
	const QUERY_TRACKING_SINGLE_USAGE = "SELECT s.*,count(t.*) AS usage,min(t.access) as first,max(access) as last FROM oc_shorty s LEFT JOIN oc_shorty_tracking t ON (s.id=t.shorty) WHERE s.id=:shorty GROUP BY s.id";
	...
	const QUERY_TRACKING_TOTAL_USAGE  = "SELECT s.*,count(t.*) AS usage,min(t.access) as first,max(access) as last FROM oc_shorty s LEFT JOIN oc_shorty_tracking t ON (s.id=t.shorty) GROUP BY s.id ORDER BY :sort";
