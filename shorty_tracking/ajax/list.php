...
{
	define ('PAGE_SIZE', 25);
	$p_shorty = Type::req_argument ( 'shorty', Type::ID,      TRUE );
	$p_offset = Type::req_argument ( 'offset', Type::INTEGER, FALSE);
...
//   ];
	if ($p_offset)
	{
		$query = \OCP\DB::prepare ( Query::CLICK_LIST_CHUNK );
		$query->bindValue(':offset', $p_offset);
	}
	else
		$query = \OCP\DB::prepare ( Query::CLICK_LIST_START );
	...
	$query = \OCP\DB::prepare ( Query::CLICK_LIST_STATS );
	$query->bindValue(':shorty', $p_shorty);
	$result = $query->execute();
	...
	\OCP\JSON::success ( [
		...
		'stats'   => $stats,
		'message' => sprintf('%s: %s',L10n::t("Number of entries"), count($chunk))
	] );
